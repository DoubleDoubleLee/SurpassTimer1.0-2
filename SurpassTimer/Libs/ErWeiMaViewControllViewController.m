...
- (void)viewDidLoad {
    ...
    self.navigationController.title = @"扫描二维码";
    ...
    UIImageView * imageView = [[UIImageView alloc]initWithFrame:CGRectMake((K_MainScreenWidth-300)/2,90, 300, 300)];
    imageView.image = [UIImage imageNamed:@"pick_bg"];
    [self.view addSubview:imageView];
    
    upOrdown = NO;
    num =0;
    _line = [[UIImageView alloc] initWithFrame:CGRectMake((K_MainScreenWidth-220)/2, 90, 220, 2)];
...
-(void)animation1
{
    if (upOrdown == NO) {
        num ++;
        _line.frame = CGRectMake((K_MainScreenWidth-220)/2, 100+2*num, 220, 2);
        if (2*num == 280) {
            upOrdown = YES;
        }
    }
